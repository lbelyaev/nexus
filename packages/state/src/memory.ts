...
export interface MemoryStore {
  appendMemoryItem: (
    item: Omit<MemoryItem, "id" | "lastAccessedAt"> & { lastAccessedAt?: string },
  ) => number;
  getMemoryItems: (
    sessionId: string,
...
export const createMemoryStore = (db: DatabaseAdapter): MemoryStore => {
  const insertStmt = db.prepare(
    `INSERT INTO memory_items (sessionId, kind, content, source, confidence, keywords, createdAt, lastAccessedAt, tokenEstimate)
     VALUES (@sessionId, @kind, @content, @source, @confidence, @keywords, @createdAt, @lastAccessedAt, @tokenEstimate)`,
  );
